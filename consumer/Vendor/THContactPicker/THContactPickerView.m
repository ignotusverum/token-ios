...
- (void)setFont:(UIFont *)font {
    ...
    self.textField.font = [UIFont fontWithName:TMFontName size:14.0];
    [self.textField sizeToFit];
    
    self.promptLabel.font = font;
    self.placeholderLabel.font = [UIFont fontWithName:TMItalicFontName size:13.0];
...
- (void)setContactViewStyle:(THContactViewStyle *)style selectedStyle:(THContactViewStyle *)selectedStyle {
    ...
    self.textField.textColor = [UIColor colorWithRed:(54.0/255.0) green:(54.0/255.0) blue:(54.0/255.0) alpha:1.0]; // RMBlackColor
...
- (void)removeContactByKey:(id)contactKey {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(contactPickerDidRemoveContact:)]) {
        if ([contactKey isKindOfClass:[NSString class]]) {
            [self.delegate contactPickerDidRemoveContact:contactKey];
        }
        else {
            [self.delegate contactPickerDidRemoveContact:[contactKey nonretainedObjectValue]];
        }
    }
...
{
    if ([self.delegate respondsToSelector:@selector(contactPickerTextField:shouldChangeCharactersInRange:replacementString:)]) {
        return [self.delegate contactPickerTextField:textField shouldChangeCharactersInRange:range replacementString:string];
    }
    return YES;
}
...
{
    if ([self.delegate respondsToSelector:@selector(contactPickerTextDidBeginEditing:)]) {
        [self.delegate contactPickerTextDidBeginEditing:textField];
    }
}
